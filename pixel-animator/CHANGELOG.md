...
- **背景**：用户反馈「绘图会出现断断续续的情况」。根因是自由绘制（铅笔 / 橡皮）时，每次 `mousemove` 只在**事件坐标处**盖一个笔刷章。鼠标快速拖动时浏览器派发的 `mousemove` 事件很稀疏，相邻两次事件之间没有任何插值，于是画出来是「断点 / 虚线 / 锯齿状跳格」。
- **修复**：`canvas-engine.js` 新增 `_strokeTo(x, y)`——记录上一个落笔的逻辑像素坐标（`_lastX/_lastY`），每次 `mousemove` 用 **Bresenham 直线算法** 从「上一点」连续插值到「当前点」，沿路径逐点盖章；`mousedown` 与 `mousemove`（铅笔/橡皮）均改走该路径，`mouseup` 时重置 `_lastX/_lastY`。橡皮同理（沿路径置 null），保证拖多快都是连笔。
- **验证（puppeteer + Edge 实机，读取逻辑像素数组 `engine.pixels`）**：
  - 铅笔：每步跳 6 / 10 个逻辑像素的快速横拖，目标行 124 个像素**全部连笔、零断点**（maxGap=0）。
  - 橡皮：先整行涂红再快速横擦，目标行**全部被擦净、零残留**（maxGap=0）。
  - 全程无 pageerror。

...
class CanvasEngine {
  ...
  // 离屏小画布（逻辑分辨率）与 ImageData 缓冲，避免逐像素 fillRect
  _ensureOffscreen() {
    ...
    if (!this._off || this._off.width !== w || this._off.height !== h) {
      ...
      this._offImg = this._offCtx.createImageData(w, h);

      this._offOnion = document.createElement('canvas');
      this._offOnion.width = w; this._offOnion.height = h;
      this._offOnionCtx = this._offOnion.getContext('2d');
      this._offOnionImg = this._offOnionCtx.createImageData(w, h);
    }
  }

  // 颜色解析缓存（像素画颜色种类极少）
  _getRGBA(hex) {
    if (this._colorCache && this._colorCache[hex]) return this._colorCache[hex];
    ...
    this._colorCache[hex] = rgba;
    return rgba;
  }

  _drawOnionLayer(frame, tint) {
    if (!frame) return;
    const w = this.width, h = this.height;
    const data = this._offOnionImg.data;
    data.fill(0);
    const a = Math.max(0, Math.min(255, Math.round(this.onionAlpha * 255)));
    let p = 0;
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        if (frame[y * w + x]) {
          data[p] = tint[0]; data[p + 1] = tint[1]; data[p + 2] = tint[2]; data[p + 3] = a;
        }
        p += 4;
      }
    }
    this._offOnionCtx.putImageData(this._offOnionImg, 0, 0);
    const ctx = this.ctx;
    ctx.imageSmoothingEnabled = false;
    ctx.drawImage(this._offOnion, 0, 0, w, h, 0, 0, this.canvas.width, this.canvas.height);
  }
  ...
  render(onionFrame = null) {
    ...
    const data = this._offImg.data;

    // 棋盘格两色：亮 #ffffff / 暗 #d9d9d9
    const light = this.showGrid;
    let p = 0;
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const c = this.pixels[y * w + x];
        if (c) {
          const rgba = this._getRGBA(c);
          data[p] = rgba[0]; data[p + 1] = rgba[1]; data[p + 2] = rgba[2]; data[p + 3] = 255;
        } else if (light) {
          if ((x + y) % 2 === 0) { data[p] = 255; data[p + 1] = 255; data[p + 2] = 255; }
          else { data[p] = 0xD9; data[p + 1] = 0xD9; data[p + 2] = 0xD9; }
          data[p + 3] = 255;
        } else {
    ...
    this._offCtx.putImageData(this._offImg, 0, 0);

    ctx.imageSmoothingEnabled = false;
    ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    ctx.drawImage(this._off, 0, 0, w, h, 0, 0, this.canvas.width, this.canvas.height);

    // 洋葱皮：前一帧（红 tint）+ 后一帧（蓝 tint）
    if (this.onionPrev || this.onionNext) {
      this._drawOnionLayer(this.onionPrev, [255, 59, 48]);
      this._drawOnionLayer(this.onionNext, [10, 132, 255]);
    }

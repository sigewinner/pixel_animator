...
(function () {
  ...
  function computePixelSize(w, h) {
    ...
    return Math.max(1, Math.min(32, ps));
  }
  ...
  // ---- 快照历史（全局：包含图层数据与所有帧） ----
  // 帧采用 copy-on-write：外层容器浅拷贝（避免 splice 改容器串味），
  // 内层帧像素数组共享引用（帧只整体替换、从不原地修改，故安全）。
  // 撤销时 restoreSnapshot 才对帧做深拷贝，使恢复后的状态独立。
  function saveSnapshot() {
    return {
      width: S.engine.width,
      height: S.engine.height,
      frames: S.anim.frames.slice(),
  ...
  // ---- RLE 行程编码（存储压缩，不改变内存表示） ----
  // 像素画大量连续同色/透明像素，RLE 压缩比极高。
  // 单像素数组编码为扁平 [count, color, count, color, ...]，整体包为 { rle:true, data:[...] }。
  function rleEncode(pixels) {
    var runs = [], i = 0, n = pixels.length;
    while (i < n) {
      var c = pixels[i], j = i + 1;
      while (j < n && pixels[j] === c) j++;
      runs.push(j - i); runs.push(c);
      i = j;
    }
    return runs;
  }
  function rleDecode(runs) {
    var out = [], k = 0;
    for (; k < runs.length; k += 2) {
      var cnt = runs[k], c = runs[k + 1], t = 0;
      for (; t < cnt; t++) out.push(c);
    }
    return out;
  }
  // 帧集合（[framePixelArray, ...]）↔ RLE
  function encodeFrames(frames) {
    return { rle: true, data: frames.map(rleEncode) };
  }
  function decodeFramesPayload(payload) {
    if (payload && payload.rle) return payload.data.map(rleDecode);
    return payload; // 旧格式：直接是数组
  }
  // 图层像素（[frame][layer] 像素数组）↔ RLE
  function encodeLayerFramePixelData(fpd) {
    return { rle: true, data: fpd.map(function (frame) { return frame.map(rleEncode); }) };
  }
  function decodeLayerFramePixelData(payload) {
    if (payload && payload.rle) return payload.data.map(function (frame) { return frame.map(rleDecode); });
    return payload;
  }
  // 把图层快照整体编码（仅压缩 framePixelData）
  function encodeLayerSnapshot(snap) {
    snap.framePixelData = encodeLayerFramePixelData(snap.framePixelData);
    return snap;
  }
  // 把图层快照整体解码（还原 framePixelData 为像素数组）
  function decodeLayerSnapshot(ld) {
    if (ld && ld.framePixelData) ld.framePixelData = decodeLayerFramePixelData(ld.framePixelData);
    return ld;
  }
  PA.rleEncode = rleEncode; PA.rleDecode = rleDecode;
  PA.encodeFrames = encodeFrames; PA.decodeFramesPayload = decodeFramesPayload;
  PA.encodeLayerFramePixelData = encodeLayerFramePixelData; PA.decodeLayerFramePixelData = decodeLayerFramePixelData;
  PA.encodeLayerSnapshot = encodeLayerSnapshot; PA.decodeLayerSnapshot = decodeLayerSnapshot;
  ...
  async function loadProject() {
    ...
    if (project) {
      ...
      var frames = PA.decodeFramesPayload(project.frames);
      S.canvasW = width;
      S.canvasH = height;
      ...
      // 暂存图层数据，待 LayerSystem 创建后恢复（保持与帧一致）；先解码 RLE
      window.__pendingLayerData = project.layerData ? PA.decodeLayerSnapshot(project.layerData) : null;
  ...
  function syncSizeSelectors(w, h) {
    ...
    var maxDim = Math.max(w, h);
    var resolutions = [16, 24, 32, 48, 64, 96, 128, 256, 512, 1080];
    var bestRes = 32;
    ...
    // 如果最接近的预设差异较大，切换到自定义模式
    if (bestDiff > 0) {
      resSel.value = 'custom';
      if (customInput) {
        customInput.style.display = '';
        customInput.value = maxDim;
      }
    } else {
    ...
    for (var j = 0; j < ratios.length; j++) {
      ...
      var targetW = parts[0] >= parts[1] ? maxDim : Math.round(maxDim * parts[0] / parts[1]);
      var targetH = parts[0] >= parts[1] ? Math.round(maxDim * parts[1] / parts[0]) : maxDim;
  ...
  // ---- 初始化 / 编排 ----
  async function init() {
    var res = PA.CanvasSize.getResolution();

...
class Animation {
  ...
  deleteFrame() {
    ...
    if (this.frames.length === 0) {
      // 允许删除到空工程：加载空白帧以保持可绘制状态
      this.current = 0;
      this.engine.loadFrame(null);
      this._renderOnion();
      if (this.onFramesChange) this.onFramesChange();
      return;
    }
  ...
  getThumbnail() {
    ...
    // 缩略图最长边封顶 256px：1:1 渲染后整体缩放，避免大分辨率生成巨型 PNG
    const maxDim = Math.max(this.width, this.height);
    const scale = Math.min(1, 256 / maxDim);
    ...
    octx.drawImage(tmp, 0, 0, this.width, this.height, 0, 0, outW, outH);
    return out.toDataURL('image/png');
  }
}
